...
//  EssentialAppTests
//
//  Created by Alberto Garcia Paul on 01/02/24.
//
...
class FeedAcceptanceTests: XCTestCase {
    ...
    func test_onFeedImageSelection_displaryComments() {
    ...
    private func launch(
        ...
        let sut = SceneDelegate(httpClient: httpClient, store: store, scheduler: .inmediateOnMainQueue)
    ...
    private func enterBackground(with store: InMemoryFeedStore) {
        let sut = SceneDelegate(httpClient: HTTPClientStub.offline, store: store, scheduler: .inmediateOnMainQueue)
